...
class Ptpelayanan_model extends CI_Model{

	public function detail_ptpelayanan($email){
		return $this->db->get_where('user_petugas',['email'=>$email])->row_array();
	}

	public function ganti_foto(){
		$ptpelayanan = $this->db->get_where('user_petugas',['email'=>$this->session->userdata('email')])->row_array();
		if($_FILES['gambar']['name']){
			...
			$this->load->library('upload', $config);
				if($this->upload->do_upload('gambar')){
					$gambar_lama = $ptpelayanan['gambar'];
