...
class Ptkeuangan_model extends CI_Model{

	public function detail_ptkeuangan($email){
		return $this->db->get_where('user_petugas',['email'=>$email])->row_array();
	}

	public function ganti_foto(){
		$ptkeuangan = $this->db->get_where('user_petugas',['email'=>$this->session->userdata('email')])->row_array();
